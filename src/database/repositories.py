...
class StationRepository:
    ...
    def update(self, station_id: int, station_data: Dict[str, Any]) -> Optional[Station]:
...
class DischargeObservationRepository:
    ...
    def bulk_create(self, observations: List[Dict[str, Any]]) -> int:
        ...
        for obs_data in observations:
            # Check if exists
            existing = self.db.query(DischargeObservation).filter(
                and_(
    ...
    def get_latest_observation(self, station_id: int, data_type: str) -> Optional[DischargeObservation]:
        """Get the most recent observation for a station."""
        return self.db.query(DischargeObservation).filter(
            and_(
                DischargeObservation.station_id == station_id,
                DischargeObservation.type == data_type
            )
        ).order_by(DischargeObservation.observed_at.desc()).first()
...
class PullConfigurationRepository:
    ...
    def get_by_id(self, config_id: int) -> Optional[PullConfiguration]:
        return self.db.query(PullConfiguration).filter(PullConfiguration.id == config_id).first()
    ...
    def update(self, config_id: int, config_data: Dict[str, Any]) -> Optional[PullConfiguration]:
    ...
    def get_stations(self, config_id: int) -> List[PullConfigurationStation]:
        return self.db.query(PullConfigurationStation).filter(
            PullConfigurationStation.config_id == config_id
        ).all()
...
class PullStationProgressRepository:
    ...
    def get_progress(self, config_id: int, station_number: str) -> Optional[PullStationProgress]:
        return self.db.query(PullStationProgress).filter(
            and_(
    ...
    def update_progress(self, config_id: int, station_number: str, last_pull_date: datetime) -> PullStationProgress:
...
class DataPullLogRepository:
    ...
    def update_log(self, log_id: int, log_data: Dict[str, Any]) -> Optional[DataPullLog]:
    ...
    def get_recent_logs(self, config_id: int, limit: int = 10) -> List[DataPullLog]:
        return self.db.query(DataPullLog).filter(
            DataPullLog.config_id == config_id
        ).order_by(DataPullLog.start_time.desc()).limit(limit).all()
...
class MasterStationRepository:
    ...
    def bulk_upsert(self, stations: List[Dict[str, Any]]) -> int:
        ...
        count = 0
        for station_data in stations:
            existing = self.db.query(MasterStation).filter(
                MasterStation.station_number == station_data['station_number']
            ).first()
...
class StationMappingRepository:
    ...
    def get_mapping(self, source_agency: str, source_id: str, target_agency: str) -> Optional[str]:
        ...
        mapping = self.db.query(StationMapping).filter(
            and_(
    ...
    def bulk_create(self, mappings: List[Dict[str, Any]]) -> int:
        ...
        for mapping_data in mappings:
            existing = self.db.query(StationMapping).filter(
                and_(
