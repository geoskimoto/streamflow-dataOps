    ...
    def get_daily_mean(self,
        ...
        try:
            ...
            for _, row in daily_means.iterrows():
                obs = {
                    'observed_at': datetime.combine(row['date'], datetime.min.time()).replace(tzinfo=pytz.UTC),
            ...
            self.logger.info(f"Calculated {len(observations)} daily mean records from EC data")
            return observations
            
        except Exception as e:
            self.logger.error(f"Error fetching EC daily mean data for {station_number}: {e}")
            raise
