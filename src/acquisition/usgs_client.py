...
class USGSClient:
        ...
        try:
            ...
            for index, row in df.iterrows():
                ...
                obs = {
                    "observed_at": index,
                    "discharge": (
                        float(row[discharge_col])
                        if pd.notna(row[discharge_col])
                        else None
                    ),
                    ...
                    "quality_code": (
                        row[quality_col]
                        if quality_col and pd.notna(row[quality_col])
                        else None
                    ),
                }
            ...
            self.logger.info(
                f"Retrieved {len(observations)} daily mean records for {station_number}"
            )
            return observations

        except Exception as e:
            self.logger.error(
                f"Error fetching USGS daily mean data for {station_number}: {e}"
            )
            raise
    ...
    def get_instantaneous(
        ...
        try:
            ...
            self.logger.info(
                f"Retrieved {len(observations)} instantaneous records for {station_number}"
            )
            return observations

        except Exception as e:
            self.logger.error(
                f"Error fetching instantaneous USGS data for {station_number}: {e}"
            )
            raise
    ...
    def get_station_info(self, station_number: str) -> Optional[Dict]:
        ...
        try:
            ...
            return {
                ...
                "drainage_area": (
                    float(row.get("drain_area_va", 0))
                    if pd.notna(row.get("drain_area_va"))
                    else None
                ),
            }
        except Exception as e:
