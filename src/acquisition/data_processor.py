...
class DataProcessor:
        ...
        for obs in observations:
            ...
            if not isinstance(obs['observed_at'], datetime):
                try:
                    ...
                    self.logger.warning(f"Invalid date format: {obs['observed_at']}, error: {e}")
    ...
    def validate_observations(self, observations: List[Dict]) -> List[Dict]:
        ...
        if removed_count > 0:
            self.logger.info(f"Removed {removed_count} invalid observations during validation")
    ...
    def process_forecast(self,
        ...
        if not forecast_data or not forecast_data.get('data'):
            self.logger.info(f"No forecast data to process for station {station_number}")
            return False
    ...
    def get_data_quality_summary(self, observations: List[Dict]) -> Dict:
        ...
        valid_discharges = [
            obs['discharge'] for obs in observations 
            if obs.get('discharge') is not None and obs['discharge'] >= 0
        ]
        ...
        return {
            ...
            'mean_discharge': sum(valid_discharges) / len(valid_discharges) if valid_discharges else None
        }
